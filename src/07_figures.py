...
def fig_a_note_map(taxonomy_map):
    ...
    ax.legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=8, title="cluster: auto-name",
              title_fontsize=8)
    fig.tight_layout()
...
def fig_b_launch_share(by_year, territory_scores):
    ...
    for c in top8_clusters:
        ...
        ax.plot(sub["year"], sub["supply_share"] * 100, marker="o", markersize=3,
                linewidth=1.5, color=CMAP(c % 20), label=f"{c}: {name}")
    ...
    ax.legend(loc="upper left", fontsize=8, title="cluster: auto-name", title_fontsize=8)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "b_launch_share_over_time.png", dpi=DPI)
    plt.close(fig)


def fig_c_demand_vs_supply(territory_scores):
    ...
    for _, row in top3.iterrows():
        ax.annotate(f"{row['cluster']}: {row['cluster_name']}",
...
def main():
    ...
    fig_a_note_map(taxonomy_map)
    fig_b_launch_share(by_year, territory_scores)
    fig_c_demand_vs_supply(territory_scores)
